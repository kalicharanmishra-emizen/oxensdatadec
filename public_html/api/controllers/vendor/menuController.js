...
/* Menu category function start */
    const categoryList = async (req,res,next) => {
        ...
        try {
            ...
            let result = await MenuCategory.find({vendorId:authUser._id}).sort({'createdAt':-1}).select('title')
            responseData={
                data:{
                    statusCode:200,
                    message:'success',
            ...
            let resultRow = MenuItem.aggregate([
                    {
                        $match:{
                            "vendorId":req.user._id,
                            category:await convortToObjID(req.body.category)
                        }
                    },
                    {
                        $lookup:{
        ...
        try {
         ...
         console.log("xero item create: ",xeroItemCreate.Items[0].ItemID);*/
            ...
            let rowData = new MenuItem({
                ...
                status:false,  // reqData.status  change  to  defaoult false 
                item_img:await fileUpload(req.files.item_img,'items/'),
                badge:reqData.badge,
                aprovedByAdmin:false,  // added for aprovel of product by admin side 
            ...
            let mailData = {
                ...
                subject: 'New Item Approval Request',
                temp: `<p>A new item named is <b> ${result?.title} </b> has been added to the system and requires approva</p>`
            }
            sendEmail(mailData)
